...
var JSON_RESOURCE_WHITELIST = ['direct_messages/events/new'];
...
Twitter.prototype.__request = function(method, path, params, callback) {
  ...
  if (method === 'post') {
    ...
    if (typeof params.media !== 'undefined') {
      formKey = 'formData';
    } else if (JSON_RESOURCE_WHITELIST.indexOf(path) > -1) {
      formKey = 'body';
    }
  
    options[formKey] = formKey == 'body' ? JSON.stringify(params) : params;
  }
